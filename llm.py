...
def filter_results(llm, query, results, scraped_data=None):
    if not results:
        return []

    system_prompt = """
    You are a Cybercrime Threat Intelligence Expert. You are given a dark web search query and a list of search results with their scraped content. 
    ...
    final_str = _generate_final_string(results, scraped_data)
    ...
    try:
        ...
        final_str = _generate_final_string(results, scraped_data, truncate=True)
        result_indices = chain.invoke({"query": query, "results": final_str})
...
def _generate_final_string(results, scraped_data=None, truncate=False):
    """
    ...
    for i, res in enumerate(results):
        ...
        base_str = f"{i+1}. {truncated_link} - {title}"
        if scraped_data and res["link"] in scraped_data:
            content = scraped_data[res["link"]]
            if len(content) > 500:
                content = content[:500] + "..."
            base_str += f"\nContent: {content}"
        
        final_str.append(base_str)
